pipeline {
    ...
    stages {
        ...
        stage('Copy Chef Resources to S3') {
            steps {
                script{
                    env.CHEF_STATE = sh(script: ''' aws s3 cp s3://swms-infra-deployment/env:/lx739q17/terraform.tfstate - | jq '.resources | .[] | select(.name=="cheff_state") | .instances | .[] | .attributes.content' ''',returnStdout: true)
